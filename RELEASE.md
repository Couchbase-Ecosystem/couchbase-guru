...
This document describes how to create a new release of `couchbase-guru`.
...
4. **`server.json`** (Docker image tags in OCI packages):
   ...
   ```json
   {
     "packages": [
       {
         "registryType": "oci",
         "identifier": "docker.io/couchbaseecosystem/couchbase-guru:0.1.1",
         ...
       }
     ]
   }
...
- All versions should be `0.1.1`
- Docker image tag should be `:0.1.1`

If versions don't match, run `./scripts/update_version.sh 0.1.1` again.
...
1. **PyPI Release**
   ...
   - Publishes to PyPI as `couchbase-guru`
   - Creates GitHub Release with changelog

2. **Docker Build**
   - Builds multi-architecture images (amd64, arm64)
   - Pushes to Docker Hub as `couchbaseecosystem/couchbase-guru`
...
- [GitHub Actions](https://github.com/Couchbase-Ecosystem/couchbase-guru/actions)

Verify the release is available on:

- [PyPI](https://pypi.org/project/couchbase-guru/)
- [Docker Hub](https://hub.docker.com/r/couchbaseecosystem/couchbase-guru)
...
./scripts/update_version.sh 0.1.1rc1
...
- PyPI: `couchbase-guru==0.1.1rc1`
- Docker Hub: `couchbaseecosystem/couchbase-guru:0.1.1rc1`
- MCP Registry: version `0.1.1rc1`
...
- Fix the issues
- Create `0.1.1rc2` and test again
- No version conflicts since the final `0.1.1` wasn't published yet!
...
```bash
# If v0.1.1 was published but release incomplete
